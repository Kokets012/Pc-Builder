...
function checkAllCompatibility($connection, $selected_components) {
    ...
    if (isset($selected_components['CPU']) && isset($selected_components['Motherboard'])) {
    ...
    if (isset($selected_components['Motherboard']) && isset($selected_components['RAM'])) {
...
function checkCPUMotherboardCompatibility($connection, $cpu_id, $motherboard_id) {
    ...
    if ($cpu['socket_type'] != $motherboard['socket_type']) {
        return "CPU socket ({$cpu['socket_type']}) does not match motherboard socket ({$motherboard['socket_type']})";
    }
...
function checkMotherboardRAMCompatibility($connection, $motherboard_id, $ram_id) {
    ...
    if ($motherboard['ram_type'] != $ram['ram_type']) {
        return "Motherboard supports {$motherboard['ram_type']} but selected RAM is {$ram['ram_type']}";
    }
