...
try {
    ...
    error_log("PC Builder Error: " . $e->getMessage());
...
<head>
    ...
    <title>EVETech PC Builder - Compatibility Checker</title>
...
<body>
    <div class="container mt-4">
        <header class="text-center mb-5">
            <h1 class="display-4 text-primary">EVETech PC Builder</h1>
                                    ...
                                    if (isset($all_cpus)) {
                                        while ($cpu = mysqli_fetch_assoc($all_cpus)) {
                                            ...
                                            echo "<option value='{$cpu['id']}' $selected>
                                                    {$cpu['brand']} {$cpu['name']} - " . CURRENCY_SYMBOL . number_format($cpu['price'], 2) . " ({$cpu['socket_type']})
                                                  </option>";
                                        }
                                    ...
                                    if (isset($all_motherboards)) {
                                        while ($mb = mysqli_fetch_assoc($all_motherboards)) {
                                            ...
                                            echo "<option value='{$mb['id']}' $selected>
                                                    {$mb['brand']} {$mb['name']} - " . CURRENCY_SYMBOL . number_format($mb['price'], 2) . " ({$mb['socket_type']}/{$mb['ram_type']})
                                                  </option>";
                                        }
                                    ...
                                    if (isset($all_ram)) {
                                        while ($ram = mysqli_fetch_assoc($all_ram)) {
                                            ...
                                            echo "<option value='{$ram['id']}' $selected>
                                                    {$ram['brand']} {$ram['name']} - " . CURRENCY_SYMBOL . number_format($ram['price'], 2) . " ({$ram['ram_type']})
                                                  </option>";
                                        }
                                    ...
                                    if (isset($all_gpus)) {
                                        while ($gpu = mysqli_fetch_assoc($all_gpus)) {
                                            ...
                                            echo "<option value='{$gpu['id']}' $selected>
                                                    {$gpu['brand']} {$gpu['name']} - " . CURRENCY_SYMBOL . number_format($gpu['price'], 2) . "
                                                  </option>";
                                        }
                ...
                <div class="card sticky-top">
                    ...
                    <div class="card-body">
                            ...
                            <div id="componentsList">
                                ...
                                if (isset($_SESSION['selected_components'])) {
                                    foreach ($_SESSION['selected_components'] as $category => $component_id) {
                                        if ($component_id > 0 && isset($db_connection)) {
                                            ...
                                            if ($component) {
                                                ...
                                                echo "<small class='text-success'>" . CURRENCY_SYMBOL . number_format($component['price'], 2) . "</small>";
                                                echo "</div>";
                                            }
                                        }
                                    }
                        ...
                        <div class="total-price">
                            <h4>Total: <?php echo CURRENCY_SYMBOL; ?><span id="totalPrice"><?php echo number_format($total_price, 2); ?></span></h4>
                        </div>
